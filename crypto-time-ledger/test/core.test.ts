...
const okTsr: VerifyTSR = async () => true;
...
test("verifyChain happy path (2 blocks)", async () => {
  const b0 = makeBlock(0, "genesis");
  const b1 = makeBlock(1, b0.hash_now);
  const r = await verifyChain([b0, b1], okTsr);
  assert.equal(r.ok, true);
});

test("verifyChain index gap", async () => {
  const b0 = makeBlock(0, "genesis");
...
test("verifyChain hash_prev mismatch", async () => {
  const b0 = makeBlock(0, "genesis");
...
test("verifyChain hash_now mismatch (tampered state)", async () => {
  const b0 = makeBlock(0, "genesis");
  const b1 = makeBlock(1, b0.hash_now);
...
test("verifyChain TSR invalid", async () => {
  const b0 = makeBlock(0, "genesis");
  const b1 = makeBlock(1, b0.hash_now);
  const failTsr: VerifyTSR = async () => false;
  const r = await verifyChain([b0, b1], failTsr);
  assert.equal(r.ok, false);
...
test("verifyChain empty chain returns ok", async () => {
  const r = await verifyChain([], okTsr);
  assert.equal(r.ok, true);
});

test("verifyChain single genesis block returns ok", async () => {
  const b0 = makeBlock(0, "genesis");
  const r = await verifyChain([b0], okTsr);
  assert.equal(r.ok, true);
});
